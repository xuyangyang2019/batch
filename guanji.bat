...
REM for /f "skip=1 tokens=1 delims= " %%i in (%fn%) do (
  echo %%i
  start cmd /k adb -s %%i shell rm -r /data/data/com.yulong.android.launcherL/databases
  ...
  REM echo 安装聚客通-酷派
  REM start cmd /k adb -s %%i install -r C:\Users\lenovo\Desktop\JuKeTool\JKT_V1.2.21-release.apk 

  start cmd /k adb -s %%i reboot 
  REM start cmd /k adb -s %%i shell reboot bootloader
  REM start cmd /k fastboot -s %%i reboot 
