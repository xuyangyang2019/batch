...
REM echo rm JuKeTool
REM adb -s %1 root
REM adb -s %1 remount
REM adb -s %1 shell rm -r /system/app/JuKeTool.apk
REM adb -s %1 shell rm -r /vendor/lib/libBugly.so 
REM adb -s %1 shell rm -r /vendor/lib/libNameProvider.so/
REM adb -s %1 shell rm -r /vendor/lib64/libBugly.so 
REM adb -s %1 shell rm -r /vendor/lib64/libNameProvider.so 
...
echo 安装xposed
adb -s %1 install -r -d E:\weike\apks\suits\XposedInstaller.apk
...
echo 安装UC
adb -s %1 install -r -d E:\weike\apks\app\UCBrowser.apk

echo 安装探探
adb -s %1 install -r -d E:\weike\apks\app\tantan.apk
